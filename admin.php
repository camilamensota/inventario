...
<body>
	...
	<!-- navLateral -->
	<section class="full-width navLateral" id="menu-lateral">
    </section>
		<script>
			$(document).ready(function(){
				// Cargamos el archivo modular
				$("#menu-lateral").load("./menu/navlateral.php", function(response, status, xhr) { 
					if (status == "success") {
						// 1. Notificar a Material Design Lite que hay contenido nuevo
						if (typeof componentHandler !== 'undefined') {
							componentHandler.upgradeAllRegistered();
						}
						
						// 2. Ejecutar la inicialización de main.js para los eventos de clic
						if(typeof window.mdl_init === 'function') {
							window.mdl_init();
						}
						
						console.log("Menú cargado y activado.");
					} else {
